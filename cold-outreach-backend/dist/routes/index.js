...
router.get('/health', async (_req, res) => {
    try {
        ...
        await database_1.prisma.$queryRaw `SELECT 1 as test`;
...
router.patch('/workflow/sessions/:sessionId/step', (req, res) => {
    ...
    try {
        ...
        apiResponse_1.ApiResponseBuilder.error(res, error instanceof Error ? error.message : 'Failed to update workflow step', 500);
    }
});
// Temporary workflow step start endpoint
router.post('/workflow/sessions/:sessionId/steps/:stepName/start', (req, res) => {
    const { sessionId, stepName } = req.params;
    const { data } = req.body;
    try {
        if (!sessionId || !stepName) {
            apiResponse_1.ApiResponseBuilder.error(res, 'Missing required parameters: sessionId or stepName', 400);
            return;
        }
        apiResponse_1.ApiResponseBuilder.success(res, {
            sessionId,
...
router.post('/workflow/sessions/:sessionId/steps/:stepName/complete', (req, res) => {
