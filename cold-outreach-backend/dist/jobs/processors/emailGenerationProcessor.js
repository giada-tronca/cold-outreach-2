...
class EmailGenerationProcessor {
    ...
    static async process(job) {
        ...
        try {
            ...
            const emailSubject = await this.generateEmailSubject(campaign.emailSubject, {
                ...prospect,
            ...
            const emailBody = await this.generateEmailBody(campaign.prompt, {
                ...prospect,
                enrichment: prospect.enrichment
            }, aiProvider, llmModelId);
     ...
     */
    static async generateEmailSubject(subjectTemplate, prospectData, aiProvider, llmModelId) {
     ...
     */
    static async generateEmailBody(bodyTemplate, prospectData, aiProvider, llmModelId) {
     ...
     */
    static async generateWithGemini(prompt, llmModelId, maxTokens = 8000, temperature = 0.7) {
        try {
     ...
     */
    static async generateWithOpenRouter(prompt, llmModelId, maxTokens = 8000, temperature = 0.7) {
        try {
     ...
     */
    static buildEmailSubjectPrompt(subjectTemplate, prospectData) {
...
SUBJECT LINE:
     ...
     */
    static buildEmailBodyPrompt(bodyTemplate, prospectData) {
        const prospectInfo = this.formatProspectData(prospectData);
        return `
...
EMAIL BODY:
     ...
     */
    static formatProspectData(prospectData) {
...
${enrichment?.prospectAnalysis ? `Prospect Analysis: ${enrichment.prospectAnalysis}` : ''}
     ...
     */
    static async updateStoredJobStatus(campaignId, prospectId = null, isCompleted = false, isFinalUpdate = false) {
        try {
            // Import here to avoid circular dependency
            const { emailGenerationJobs } = await Promise.resolve().then(() => __importStar(require('@/routes/email-generation')));
            ...
            if (jobEntry) {
                ...
                if (isFinalUpdate || prospectId === null) {
                    ...
                    const prospectIndex = job.prospects?.findIndex((p) => p.id === prospectId.toString()) ?? -1;
                    if (prospectIndex !== -1 && job.prospects) {
                        ...
                        const completed = job.prospects.filter((p) => p.status === 'completed').length;
                        const failed = job.prospects.filter((p) => p.status === 'failed').length;
     ...
     */
    static async checkAndGenerateCSV(campaignId, userId) {
        try {
            // Check how many prospects have been processed
            const allProspects = await database_1.prisma.prospect.findMany({
                where: { campaignId },
                include: {
                    enrichment: true,
                    generatedEmail: true
     ...
     */
    static async generateEmailCSV(campaignId, prospects) {
        try {
            // Create CSV content
            const csvHeaders = [
                'Name',
            ...
            const csvRows = prospects.map(prospect => {
                ...
                if (prospect.enrichment?.techStack) {
                    const techStack = prospect.enrichment.techStack;
