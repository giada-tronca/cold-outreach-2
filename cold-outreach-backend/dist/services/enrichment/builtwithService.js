...
class BuiltWithService {
     ...
     */
    static async getTechStack(domain) {
        try {
            ...
            const scrapedData = await firecrawlService_1.FirecrawlService.scrapeCompanyWebsite(builtwithUrl);
     ...
     */
    static parseBuiltWithContent(content) {
        const technologies = {};
        try {
     ...
     */
    static categorizeTechnologies(foundTechs, technologies) {
        ...
        for (const tech of foundTechs) {
            ...
            for (const [category, techList] of Object.entries(categoryMappings)) {
                if (techList.some(categoryTech => tech.toLowerCase().includes(categoryTech.toLowerCase()) ||
     ...
     */
    static fallbackTechParsing(content) {
        const technologies = {};
        ...
        for (const [category, techs] of Object.entries(techPatterns)) {
            ...
            for (const tech of techs) {
                ...
                for (const pattern of patterns) {
                    if (contentLower.includes(pattern)) {
                        const properName = tech.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
            ...
            if (foundTechs.length > 0) {
                technologies[category] = foundTechs;
            }
        }
     ...
     */
    static async getTechTrends(domain) {
        try {
     ...
     */
    static analyzeTechTrends(technologies) {
        ...
        const trends = technologies.filter(tech => ['JavaScript', 'CSS', 'HTML', 'React', 'Node.js'].some(t => tech.toLowerCase().includes(t.toLowerCase())));
        const emerging = technologies.filter(tech => modernTechs.some(t => tech.toLowerCase().includes(t.toLowerCase())));
        const declining = technologies.filter(tech => legacyTechs.some(t => tech.toLowerCase().includes(t.toLowerCase())));
        return { trends, emerging, declining };
    }
    /**
     * Get company technology summary
     */
    static async getTechSummary(domain) {
        try {
            const techStack = await this.getTechStack(domain);
            const summary = [];
     ...
     */
    static async getBuiltWithSummary(domain, aiProvider = 'openrouter') {
        try {
     ...
     */
    static formatBuiltWithDataForAI(scrapedData, domain) {
        const sections = [];
