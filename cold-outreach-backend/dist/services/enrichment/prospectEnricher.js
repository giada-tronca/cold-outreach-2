...
exports.ProspectEnricher = void 0;
...
class ProspectEnricher {
     ...
     */
    static async enrichExistingProspect(prospectId, linkedinUrl, options = {}) {
        try {
            ...
            const updatedProspect = await this.createOrUpdateProspect(existingProspect, profileData, linkedinUrl, existingProspect.email, aiSummary);
     ...
     */
    static async enrichWithProxycurl(linkedinUrl, options = {}) {
        try {
            ...
            if (existingProspect) {
                ...
                const enrichment = await database_1.prisma.prospectEnrichment.findUnique({
            ...
            const prospect = await this.createOrUpdateProspect(existingProspect, profileData, linkedinUrl, email, aiSummary);
            ...
            throw new errors_1.DatabaseError(`LinkedIn enrichment failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * Fetch LinkedIn profile data from Proxycurl API
     */
    static async fetchLinkedInProfile(linkedinUrl) {
        try {
            ...
            const response = await axios_1.default.get('https://nubela.co/proxycurl/api/v2/linkedin', {
                headers: {
     ...
     */
    static extractEmailFromProfile(profileData, linkedinUrl) {
     ...
     */
    static extractPhoneFromProfile(profileData) {
     ...
     */
    static async findExistingProspect(email, phone) {
        ...
        return await database_1.prisma.prospect.findFirst({
     ...
     */
    static async generateAISummary(profileData, aiProvider) {
        try {
     ...
     */
    static async generateSummaryWithGemini(profileSummary) {
        try {
     ...
     */
    static async generateSummaryWithOpenRouter(profileSummary) {
        const maxRetries = 3;
        let lastError = null;
     ...
     */
    static formatProfileDataForAI(profileData) {
        const parts = [];
     ...
     */
    static generateBasicSummary(profileData) {
        const parts = [];
     ...
     */
    static async enrichWithCompanyData(prospectId, options = {}) {
        try {
            ...
            return {
                ...
                data: {
                    profileData: null, // Not applicable for company enrichment
     ...
     */
    static generateCompanyWebsiteUrl(companyName) {
        try {
            // Clean company name
     ...
     */
    static async enrichWithTechStack(prospectId, options = {}) {
        try {
            ...
            const builtwithData = await firecrawlService_1.FirecrawlService.scrapeCompanyWebsite(builtwithUrl);
            ...
            return {
                ...
                data: {
                    profileData: null, // Not applicable for tech stack enrichment
     ...
     */
    static async extractCompanyDomain(prospect) {
        try {
            // 1. Check if company website is already in additional data
            if (prospect.additionalData?.companyWebsite) {
     ...
     */
    static generateCompanyDomain(companyName) {
        try {
            // Clean company name
            const cleanName = companyName
                .toLowerCase()
     ...
     */
    static async extractTechStackWithAI(builtwithContent, aiProvider) {
        try {
     ...
     */
    static async generateTechStackAnalysis(prompt, aiProvider) {
        try {
            if (aiProvider === 'gemini') {
     ...
     */
    static async generateTechStackWithGemini(prompt) {
        try {
            const apiKey = await apiConfigurationService_1.ApiConfigurationService.getApiKey('geminiApiKey');
            const response = await axios_1.default.post(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-exp:generateContent?key=${apiKey}`, {
                contents: [{
     ...
     */
    static async generateTechStackWithOpenRouter(prompt) {
        const maxRetries = 3;
        let lastError = null;
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
     ...
     * Parse AI response to extract structured tech stack data
     */
    static parseTechStackResponse(aiResponse) {
        try {
     ...
     */
    static async createOrUpdateProspect(existingProspect, profileData, linkedinUrl, email, aiSummary) {
        const prospectData = {
        ...
        if (existingProspect) {
            ...
            prospect = await database_1.prisma.prospect.update({
            ...
            // Create new prospect
            prospect = await database_1.prisma.prospect.create({
                data: prospectData
            });
        }
     ...
     */
    static async enrichWithProspectAnalysis(prospectId, options = {}) {
        try {
            ...
            return {
                ...
                data: {
                    profileData: null, // Not applicable for prospect analysis
     ...
     */
    static prepareCombinedEnrichmentData(prospect, enrichmentData) {
        const sections = [];
        ...
        if (enrichmentData.techStack && Array.isArray(enrichmentData.techStack)) {
            ...
            enrichmentData.techStack.forEach((tech) => {
     ...
     */
    static async generateProspectAnalysisWithAI(combinedData, aiProvider, llmModelId) {
        try {
     ...
     */
    static async generateProspectAnalysis(prompt, aiProvider, llmModelId) {
        try {
            if (aiProvider === 'gemini') {
     ...
     */
    static async generateProspectAnalysisWithGemini(prompt) {
        try {
            const apiKey = await apiConfigurationService_1.ApiConfigurationService.getApiKey('geminiApiKey');
            const response = await axios_1.default.post(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-exp:generateContent?key=${apiKey}`, {
                contents: [{
     ...
     */
    static async generateProspectAnalysisWithOpenRouter(prompt, llmModelId) {
        const maxRetries = 3;
        let lastError = null;
        // Determine the model to use based on llmModelId
        let modelId = 'openai/o1-mini';
        let modelName = 'o1-mini';
        let requestBody = {};
        if (llmModelId) {
            switch (llmModelId) {
...
exports.ProspectEnricher = ProspectEnricher;
