...
exports.ProxycurlService = void 0;
...
class ProxycurlService {
    ...
    static async createAxiosInstance() {
        const apiKey = await apiConfigurationService_1.ApiConfigurationService.getApiKey('proxycurlApiKey');
     ...
     */
    static async enrichPersonProfile(linkedinUrl, aiProvider, llmModelId) {
        try {
            ...
            const response = await this.retryRequest(() => axiosInstance.get('/api/v2/linkedin', {
                params: {
            ...
            const creditCost = response.headers['x-proxycurl-credit-cost'];
            ...
            if (axios_1.default.isAxiosError(error) && error.response) {
                ...
                throw new errors_1.AppError(`Proxycurl API error (${status}): ${message}`);
            }
            throw new errors_1.AppError(`Proxycurl enrichment failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     ...
     */
    static async generateLinkedInSummaryWithAI(profileData, aiProvider, llmModelId) {
        try {
     ...
     */
    static formatAllLinkedInDataForAI(profile) {
        const sections = [];
        // === BASIC INFORMATION ===
        ...
        if (profile.first_name)
            sections.push(`First Name: ${profile.first_name}`);
        if (profile.last_name)
            sections.push(`Last Name: ${profile.last_name}`);
        if (profile.headline)
            sections.push(`Professional Headline: ${profile.headline}`);
        if (profile.public_identifier)
            sections.push(`LinkedIn ID: ${profile.public_identifier}`);
        // === LOCATION ===
        if (profile.city || profile.state || profile.country || profile.country_full_name) {
            sections.push('\n=== LOCATION ===');
            if (profile.city)
                sections.push(`City: ${profile.city}`);
            if (profile.state)
                sections.push(`State: ${profile.state}`);
            if (profile.country)
                sections.push(`Country: ${profile.country}`);
            if (profile.country_full_name)
                sections.push(`Country Full: ${profile.country_full_name}`);
        }
        ...
        if (profile.experiences && profile.experiences.length > 0) {
            ...
            profile.experiences.forEach((exp, index) => {
                ...
                if (exp.company_linkedin_profile_url)
                    sections.push(`  Company LinkedIn: ${exp.company_linkedin_profile_url}`);
                if (exp.location)
        ...
        if (profile.education && profile.education.length > 0) {
            ...
            profile.education.forEach((edu, index) => {
                sections.push(`\nEducation ${index + 1}:`);
                if (edu.school)
                    sections.push(`  School: ${edu.school}`);
                if (edu.degree_name)
                    sections.push(`  Degree: ${edu.degree_name}`);
                if (edu.field_of_study)
                    sections.push(`  Field of Study: ${edu.field_of_study}`);
                if (edu.grade)
                    sections.push(`  Grade: ${edu.grade}`);
                if (edu.school_linkedin_profile_url)
                    sections.push(`  School LinkedIn: ${edu.school_linkedin_profile_url}`);
        ...
        if (profile.certifications && profile.certifications.length > 0) {
            ...
            profile.certifications.forEach((cert, index) => {
                ...
                if (cert.name)
                    sections.push(`  Name: ${cert.name}`);
                if (cert.authority)
                    sections.push(`  Authority: ${cert.authority}`);
                if (cert.license_number)
                    sections.push(`  License Number: ${cert.license_number}`);
                if (cert.url)
                ...
                if (startDate)
                    sections.push(`  Valid: ${startDate} - ${endDate}`);
            });
        }
        ...
        if (profile.accomplishment_organisations && profile.accomplishment_organisations.length > 0) {
            ...
            profile.accomplishment_organisations.forEach((org, index) => {
                ...
                if (org.org_name)
                    sections.push(`  Organization: ${org.org_name}`);
                if (org.title)
                    sections.push(`  Title: ${org.title}`);
                if (org.description)
        ...
        if (profile.accomplishment_publications && profile.accomplishment_publications.length > 0) {
            ...
            profile.accomplishment_publications.forEach((pub, index) => {
                sections.push(`\nPublication ${index + 1}:`);
                if (pub.name)
                    sections.push(`  Title: ${pub.name}`);
                if (pub.publisher)
                    sections.push(`  Publisher: ${pub.publisher}`);
                if (pub.url)
                    sections.push(`  URL: ${pub.url}`);
                if (pub.description)
                    sections.push(`  Description: ${pub.description}`);
                const pubDate = pub.published_on ? this.formatDate(pub.published_on) : null;
                if (pubDate)
                    sections.push(`  Published: ${pubDate}`);
            });
        }
        ...
        if (profile.accomplishment_patents && profile.accomplishment_patents.length > 0) {
            ...
            profile.accomplishment_patents.forEach((patent, index) => {
                sections.push(`\nPatent ${index + 1}:`);
                if (patent.title)
                    sections.push(`  Title: ${patent.title}`);
                if (patent.issuer)
                    sections.push(`  Issuer: ${patent.issuer}`);
                if (patent.patent_number)
                    sections.push(`  Patent Number: ${patent.patent_number}`);
                if (patent.application_number)
                    sections.push(`  Application Number: ${patent.application_number}`);
                if (patent.url)
                    sections.push(`  URL: ${patent.url}`);
                if (patent.description)
                    sections.push(`  Description: ${patent.description}`);
        ...
        if (profile.accomplishment_courses && profile.accomplishment_courses.length > 0) {
            ...
            profile.accomplishment_courses.forEach((course, index) => {
                sections.push(`\nCourse ${index + 1}:`);
                if (course.name)
                    sections.push(`  Name: ${course.name}`);
                if (course.number)
                    sections.push(`  Course Number: ${course.number}`);
            });
        }
        ...
        if (profile.accomplishment_test_scores && profile.accomplishment_test_scores.length > 0) {
            ...
            profile.accomplishment_test_scores.forEach((test, index) => {
                sections.push(`\nTest ${index + 1}:`);
                if (test.name)
                    sections.push(`  Test: ${test.name}`);
                if (test.score)
                    sections.push(`  Score: ${test.score}`);
                if (test.description)
                    sections.push(`  Description: ${test.description}`);
        ...
        if (profile.volunteer_work && profile.volunteer_work.length > 0) {
            ...
            profile.volunteer_work.forEach((vol, index) => {
                ...
                if (vol.title)
                    sections.push(`  Title: ${vol.title}`);
                if (vol.company)
                    sections.push(`  Organization: ${vol.company}`);
                if (vol.cause)
                    sections.push(`  Cause: ${vol.cause}`);
                if (vol.description)
                    sections.push(`  Description: ${vol.description}`);
        ...
        if (profile.activities && profile.activities.length > 0) {
            ...
            profile.activities.forEach((activity, index) => {
                sections.push(`\nActivity ${index + 1}:`);
                if (activity.title)
                    sections.push(`  Title: ${activity.title}`);
                if (activity.link)
                    sections.push(`  Link: ${activity.link}`);
                if (activity.activity_status)
                    sections.push(`  Status: ${activity.activity_status}`);
            });
        }
        // === ARTICLES ===
        if (profile.articles && profile.articles.length > 0) {
            ...
            profile.articles.forEach((article, index) => {
                ...
                if (article.link)
                    sections.push(`  Link: ${article.link}`);
                if (article.author)
                    sections.push(`  Author: ${article.author}`);
        ...
        if (profile.groups && profile.groups.length > 0) {
            ...
            profile.groups.forEach((group, index) => {
                sections.push(`\nGroup ${index + 1}:`);
                if (group.name)
                    sections.push(`  Name: ${group.name}`);
                if (group.url)
                    sections.push(`  URL: ${group.url}`);
            });
        }
     ...
     */
    static async generateSummaryWithAI(prompt, aiProvider, llmModelId) {
        try {
    ...
    static async generateSummaryWithGemini(prompt, llmModelId) {
        try {
            ...
            const response = await axios_1.default.post(`https://generativelanguage.googleapis.com/v1beta/models/${llmModelId}:generateContent?key=${apiKey}`, {
                contents: [{
                        parts: [{
                                text: prompt
                            }]
     ...
     */
    static async enrichCompanyProfile(companyLinkedinUrl) {
        try {
            console.log(`🔍 [Proxycurl]: Enriching company profile: ${companyLinkedinUrl}`);
            const axiosInstance = await this.createAxiosInstance();
            const config = await apiConfigurationService_1.ApiConfigurationService.getModelConfiguration();
            const response = await this.retryRequest(() => axiosInstance.get('/api/linkedin/company', {
                params: {
            ...
            throw new errors_1.AppError(`Proxycurl company enrichment failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * Format location string
     */
    static formatLocation(city, state, country) {
     ...
     */
    static extractCurrentPosition(experiences) {
     ...
     */
    static formatExperiences(experiences) {
        if (!experiences)
            return undefined;
     ...
     */
    static formatCompanyLocation(hq) {
        if (!hq)
            return undefined;
...
exports.ProxycurlService = ProxycurlService;
// Updated to use the official Proxycurl API endpoint
ProxycurlService.BASE_URL = 'https://nubela.co/proxycurl';
