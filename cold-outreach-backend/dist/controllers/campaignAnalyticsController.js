...
exports.exportCampaignData = exportCampaignData;
...
async function getCampaignAnalytics(req, res) {
    try {
        ...
        const enrichedProspects = campaign.prospects.filter(p => ['ENRICHED', 'GENERATING', 'COMPLETED'].includes(p.status)).length;
        const generatedEmails = campaign.prospects.filter(p => p.status === 'COMPLETED').length;
        const failedProspects = campaign.prospects.filter(p => p.status === 'FAILED').length;
        const pendingProspects = campaign.prospects.filter(p => p.status === 'PENDING').length;
        const processingProspects = campaign.prospects.filter(p => ['ENRICHING', 'GENERATING'].includes(p.status)).length;
        // Calculate rates
        const successRate = totalProspects > 0 ? (generatedEmails / totalProspects) * 100 : 0;
        const enrichmentRate = totalProspects > 0 ? (enrichedProspects / totalProspects) * 100 : 0;
        const failureRate = totalProspects > 0 ? (failedProspects / totalProspects) * 100 : 0;
        // Batch statistics
        ...
        throw new errors_1.DatabaseError('Failed to fetch campaign analytics');
    }
}
/**
...
async function getCampaignMetrics(req, res) {
    try {
        ...
        throw new errors_1.DatabaseError('Failed to fetch campaign metrics');
    }
}
/**
...
async function compareCampaigns(req, res) {
    try {
        ...
        throw new errors_1.DatabaseError('Failed to compare campaigns');
    }
}
/**
...
async function exportCampaignData(req, res) {
    try {
        ...
        throw new errors_1.DatabaseError('Failed to prepare export');
    }
}
