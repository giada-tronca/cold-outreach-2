import { CSVParserService, CSVParseResult } from './csvParser';
import { DataMapperService, MappingResult, ProspectData, MappingConfig } from './dataMapper';
import { BadRequestError } from '@/utils/errors';

export interface BatchConfig {
    chunkSize?: number; // Number of rows to process in each batch
    maxConcurrency?: number; // Maximum number of concurrent batch jobs
    retryAttempts?: number; // Number of retry attempts for failed batches
    retryDelay?: number; // Delay between retries in milliseconds
    skipErrors?: boolean; // Whether to skip rows with errors
    saveProgressInterval?: number; // How often to save progress (in batches)
}
...
export interface BatchProgress {
    ...
    status: 'parsing' | 'mapping' | 'processing' | 'saving' | 'completed' | 'failed';
    message: string;
    startTime: Date;
...
export class BatchProcessorService {
    private static readonly DEFAULT_CONFIG: Required<BatchConfig> = {
        chunkSize: 500,
        maxConcurrency: 3,
        retryAttempts: 3,
        retryDelay: 1000,
    ...
    static async processCSVFile(
        ...
        try {
            ...
            if (parseResult.errors.length > 0 && !config.skipErrors) {
                throw new BadRequestError(`CSV parsing failed with ${parseResult.errors.length} errors`);
            }
            ...
            const batchResults = await this.processBatchChunks(
                batchChunks,
                options.campaignId,
                config,
                (chunkProgress) => {
     ...
     */
    private static createBatchChunks(prospects: ProspectData[], chunkSize: number): ProspectData[][] {
        const chunks: ProspectData[][] = [];
     ...
     */
    private static async processBatchChunks(
        chunks: ProspectData[][],
        campaignId: number,
        ...
        const results: BatchChunkResult[] = [];
        let currentChunk = 0;
        let processedRows = 0;
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            const chunkPromise = this.processChunk(
                ...
                const errorResult: BatchChunkResult = {
                    chunkId,
                    startRow,
                    endRow,
                    status: 'failed',
     ...
     */
    private static createInitialProgress(batchId: string, startTime: Date): BatchProgress {
        return {
            batchId,
            currentChunk: 0,
            totalChunks: 0,
     ...
     */
    private static estimateCompletion(startTime: Date, progress: number): Date {
     ...
     */
    static getBatchStats(result: BatchResult) {
        return {
