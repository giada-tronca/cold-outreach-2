...
export class ProspectEnrichmentProcessor {
    ...
    static async process(job: Job<ProspectEnrichmentJobData>): Promise<JobResult> {
        ...
        try {
            ...
            if (!isExistingProspect) {
                await prisma.cOProspectEnrichments.create({
                    data: {
                        prospectId: prospect.id,
            ...
            if (enabledServices.proxycurl && (prospect.linkedinUrl || linkedinUrl)) {
                ...
                if (isExistingProspect && existingProspectInfo?.enrichmentRecord?.linkedinSummary) {
                    ...
                    try {
                        ...
                        const linkedinData = await ProxycurlService.enrichPersonProfile(urlToUse, aiProvider, llmModelId);
                ...
                if (isExistingProspect && existingProspectInfo?.enrichmentRecord?.companySummary) {
                    ...
                    try {
                        ...
                        if (companyWebsite) {
                            ...
                            const scrapedData = await FirecrawlService.scrapeCompanyWebsite(companyWebsite);
                ...
                if (isExistingProspect && existingProspectInfo?.enrichmentRecord?.builtwithSummary) {
                    ...
                    try {
                        ...
                        if (companyWebsite) {
                            ...
                            const domain = companyWebsite.replace(/^https?:\/\//, '').replace(/^www\./, '').split('/')[0] as string;

                            console.log(`🔍 [Enrichment]: Analyzing tech stack for domain: ${domain}`);

                            const builtwithData = await BuiltWithService.getTechStack(domain as string);
                            if (builtwithData && Object.keys(builtwithData).length > 0) {
                                ...
                                // Generate tech stack summary
                                builtwithSummary = await this.generateTechStackSummary(JSON.stringify(builtwithData, null, 2), domain as string, aiProvider, llmModelId, isExistingProspect, existingProspectInfo);
     ...
     */
    private static async generateLinkedInSummary(linkedinData: any, aiProvider: 'gemini' | 'openrouter', llmModelId?: string, isExistingProspect: boolean = false, existingProspectInfo: any = null): Promise<string> {
...
Please provide a well-structured, professional summary that would be useful for sales outreach.`;
        ...
        if (promptRecord?.linkedinSummaryPrompt) {
            prompt = replaceTemplateVariables(promptRecord.linkedinSummaryPrompt, {
     ...
     */
    private static async generateTechStackSummary(builtwithData: string, domain: string, aiProvider: 'gemini' | 'openrouter', llmModelId?: string, isExistingProspect: boolean = false, existingProspectInfo: any = null): Promise<string> {
        try {
...
Please provide a well-structured summary that highlights the company's technical sophistication and potential technology needs.`;
            ...
            try {
                ...
                if (promptRecord?.techStackPrompt) {
                    techStackPrompt = replaceTemplateVariables(promptRecord.techStackPrompt, {
     ...
     */
    private static async generateProspectAnalysis(prospect: any, enrichmentData: any, aiProvider: 'gemini' | 'openrouter', llmModelId?: string, isExistingProspect: boolean = false, existingProspectInfo: any = null): Promise<string> {
        try {
...
Focus on actionable insights that would help in sales outreach and relationship building.`;
            ...
            try {
                ...
                if (promptRecord?.prospectAnalysisPrompt) {
                    ...
                    prospectAnalysisPrompt = replaceTemplateVariables(promptRecord.prospectAnalysisPrompt, {
...
LinkedIn URL: ${prospect.linkedinUrl || 'Not available'}
     ...
     */
    private static async generateCompanySummary(formattedContent: string, companyWebsite: string, aiProvider: 'gemini' | 'openrouter', llmModelId?: string, isExistingProspect: boolean = false, existingProspectInfo: any = null): Promise<string> {
        try {
...
${formattedContent}`;
            ...
            try {
                ...
                if (promptRecord?.companySummaryPrompt) {
                    companySummaryPrompt = replaceTemplateVariables(promptRecord.companySummaryPrompt, {
...
Skills: ${data.skills?.join(', ') || 'Not available'}
     ...
     */
    private static async generateSummaryWithGeminiAndTrack(prompt: string, llmModelId?: string, requestType?: string): Promise<{ summary: string; requestData: any; responseData: any }> {
        let requestData: any;
     ...
     */
    private static async generateSummaryWithOpenRouterAndTrack(prompt: string, llmModelId?: string, requestType?: string): Promise<{ summary: string; requestData: any; responseData: any }> {
        let requestBody: any;
        let model: string = '';
